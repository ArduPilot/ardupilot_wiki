...
In addition, if :ref:`WP_RFND_USE<WP_RFND_USE>` is also set to 1, the rangefinder will be used instead of the terrain database during RTL. Of course the ``RTL_ALT`` must be within the rangefinder's operating range and it must be healthy.
...
Note that because it does not immediately have access to terrain data in this situation it will perform a normal RTL interpreting the ``RTL_ALT`` as an altitude-above-home regardless of whether :ref:`RTL_ALT_TYPE<RTL_ALT_TYPE>` has been set to "1" or not.

 ...
 */
// @ts-nocheck
// One wrapper around the whole file: the glue names some seventy variables at
// its top level, and in the worker any of those the worker later declares for
// itself would be a redeclaration that stops the worker starting at all.
var ApZstd = (function () {
...
return (function () {
  ...
  // The largest page the caches will hold (sw.js STORE_LIMIT_BYTES). A frame
  // header asking for more than that is not one this build wrote.
  var MAX_PAGE_BYTES = 12 * 1024 * 1024;
  ...
  /** The page size a zstd frame header claims, or 0 where it claims none.
   * The same fields fzstd reads, read first: fzstd allocates from them. */
  function declaredSize(bytes) {
    if (bytes.length < 5 || bytes[0] !== 0x28 || bytes[1] !== 0xb5 ||
        bytes[2] !== 0x2f || bytes[3] !== 0xfd) { return 0; }
    var flags = bytes[4];
    var single = (flags >> 5) & 1, dictFlag = flags & 3, sizeFlag = flags >> 6;
    var at = 6 - single + (dictFlag === 3 ? 4 : dictFlag);
    var width = sizeFlag ? (1 << sizeFlag) : single;
    if (!width || at + width > bytes.length) { return 0; }
    var size = 0;
    for (var i = width - 1; i >= 0; i--) { size = (size * 256) + bytes[at + i]; }
    return sizeFlag === 1 ? size + 256 : size;
  }

  /** The page that `delta` encodes against `base`, in JavaScript. */
  function patchJs(delta, base) {
    // The header is the delta's own claim about the page, and the decoder
    // allocates from it as it reads: it is bounded before it is handed over.
    var want = declaredSize(delta);
    if (!(want > 0)) { throw new Error('zstd delta carries no content size'); }
    if (want > MAX_PAGE_BYTES) {
      throw new Error('zstd delta rebuilds more than a page can be');
    }
  ...
  function patch(delta, base) {
    ...
    // Every allocation is freed by the one finally, including the ones a
    // failed copy into the heap would otherwise leave behind.
    var src = 0, dict = 0, out = 0, dctx = 0;
    function alloc(n) {
      var at = malloc(n);
      if (!at && n) { throw new Error('zstd has no room to rebuild the page'); }
      return at;
    }
    try {
      src = alloc(delta.byteLength);
      Module.HEAPU8.set(delta, src);
      dict = alloc(base.byteLength);
      ...
      if (size <= 0) { throw new Error('zstd delta carries no content size'); }
      if (size > MAX_PAGE_BYTES) { throw new Error('zstd delta rebuilds more than a page can be'); }
      out = alloc(size);
      dctx = Module['_ZSTD_createDCtx']();
      ...
      if (dctx) { Module['_ZSTD_freeDCtx'](dctx); }

    ...
    for key, value in PARAMETER_SITE.items():
        if key == 'AP_Periph':  # workaround until create a versioning for AP_Periph in firmware server
            ...
            if site == key or site is None:
                ...
                ):  # To main the original script approach instead of the build_parameters.py approach.  # noqa: E501
                ...
                if (
                    ...
                    vehicle_json_file = os.getcwd() + '/../new_params_mversion/%s/parameters-%s.json' % (
                        "AntennaTracker",
                        "AntennaTracker",
                    )  # noqa: E501
                else:
                ...
                for filename in new_parameters_files:
                    ...
                    try:
                        ...
                        if not os.path.isfile(new_file):
                            ...
                            filename.replace("new_params_mversion", "old_params_mversion")
                        ):  # The cached file exists?  # noqa: E501
                            # Temporary debug messages to help with cache tasks.
                            debug(
                                "Check cache: %s against %s"
                                % (filename, filename.replace("new_params_mversion", "old_params_mversion"))
                            )  # noqa: E501
                            debug(
                                "Check cache with filecmp.cmp: %s"
                                % filecmp.cmp(filename, filename.replace("new_params_mversion", "old_params_mversion"))
                            )  # noqa: E501
                            debug(
                                "Check cache with sha256: %s"
                                % is_the_same_file(filename, filename.replace("new_params_mversion", "old_params_mversion"))
                            )  # noqa: E501

                            if (
                                ("parameters.rst" in filename)
                                or (not filecmp.cmp(filename, filename.replace("new_params_mversion", "old_params_mversion")))
                            ):  # It is different?  OR is this one the latest. | Latest file must be built every time in order to enable Sphinx create the correct references across the wiki.  # noqa: E501
...
def cache_parameters_files(site=None):
    ...
    """
    for key, value in PARAMETER_SITE.items():
        if (site == key or site is None) and (
            key != 'AP_Periph'
        ):  # and (key != 'AP_Periph') workaround until create a versioning for AP_Periph in firmware server # noqa: E501
            try:
    ...
    for platform_key in sorted(features_by_platform.keys(), key=lambda x: x.lower()):
        ...
        for feature in platform_features:
            ...
            try:
                ...
                progress("feature %s (%s,%s) not in build_options.py" % (feature, platform_key, vehicletype))
                continue
            if feature_in:
        ...
        sorted_platform_features = sorted(sorted_platform_features_not_in, key=lambda x: x[0] + x[1]) + sorted(
            sorted_platform_features_in, key=lambda x: x[0] + x[1]
        )

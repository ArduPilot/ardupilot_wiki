...
RC Input
========
The default RC input is configured on the UART6 RX6 input and can be used for all ArduPilot supported unidirectional receiver protocols. The SBus input on the HD VTX connector is tied to RX6 so do not connect anything simultaneously with anything else on RX6.

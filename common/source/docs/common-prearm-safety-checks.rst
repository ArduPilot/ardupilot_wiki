...
.. table:: Pre-Arm Failure Messages (Copter/Heli Only)
    ...
    Check PILOT_SPEED_UP                                    PILOT_SPEED_UP set too low                          Increase ``PILOT_SPEED_UP`` to a positive number (e.g. 100 = 1m/s).  See :ref:`AltHold mode<altholdmode>`
    ...
    RTL_ALT_TYPE is above-terrain but RTL_ALT>RNGFND_MAX    RTL return altitude above rangefinder range         Reduce ``RTL_ALT`` to less than RNGFNDx_MAX. See :ref:`Terrain Following<terrain-following>`

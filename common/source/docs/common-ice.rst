...
To test your throttle servo limits safely, you can set :ref:`ICE_OPTIONS<ICE_OPTIONS>` bit 2 (set to a value of 4) to allow throttle control while disarmed in Manual mode, and it does not require the engine to be considered "running". If you do not want this option long-term, be sure to disable it after testing.
...
.. warning:: If you set :ref:`ICE_IDLE_PCT<ICE_IDLE_PCT>` to a value greater than zero, the engine will be allowed to start while disarmed. If you do not want this, set :ref:`ICE_OPTIONS<ICE_OPTIONS>` bit 3 (set to a value of 8) to prevent starting while disarmed. See `Controlling the Engine`_ below for more information.
...
- If :ref:`ICE_OPTIONS<ICE_OPTIONS>` bit 0 is set (set to a value of 1), the engine will automatically disable during RC failsafe.
...
By default, the engine cannot be started while disarmed. If you want to allow it to start while disarmed, either set :ref:`ICE_IDLE_PCT<ICE_IDLE_PCT>` to a value greater than zero, or set :ref:`ICE_OPTIONS<ICE_OPTIONS>` bit 2 (set to a value of 4) to allow throttle control while disarmed in Manual mode.

If you would like to use :ref:`ICE_IDLE_PCT<ICE_IDLE_PCT>`, but *don't* want to allow starting while disarmed, set :ref:`ICE_OPTIONS<ICE_OPTIONS>` bit 3 (set to a value of 8).
...
.. tabs::
   ...
   .. tab:: ArduPilot prior to 4.6
      ...
      - :ref:`ICE_PWM_STRT_OFF<ICE_PWM_STRT_OFF>` is the PWM value sent to the starter when not trying to start the engine

...
`Aeron Systems <https://www.aeronsystems.com/>`__ develops high-reliability embedded hardware solutions for safety-critical, and mission-critical applications.
With deep expertise in MEMS, FOG, sensor fusion, inertial navigation, and rugged embedded electronics, Aeron supports aircraft, UAVs, autonomous systems, tactical land
vehicles and marine platforms with advanced navigation and control technologies. Aeron solutions are engineered to perform in demanding airborne and high-dynamics environments
meeting global aerospace quality standards.
...
Here's a detailed report of GNSS Denied Navigation using PLX3 integrated with Ardupilot. Don't forget to watch the `full flight video in GNSS Denied <https://www.aeronsystems.com/articles/plx3-x-autopilot-for-autonomous-uav--gnss-denied-operations/>`__.

Before you Start
================

Setting up the PLX3 with ArduPilot requires preparing the device itself and then connecting it to the autopilot. The PLX3 is configured **before** it ever talks to ArduPilot, using the Aeron's **Aeron INS Connect Suite (AICS)** PC tool. ArduPilot's driver does not configure
the device. There is no autopilot-side configuration push at boot.
...
Unlike some other External AHRS devices, the PLX3 is **configured via the Aeron INS Connect Suite (AICS) PC tool prior to use, not by ArduPilot at runtime**.

Connect the PLX3 to a Windows/Linux PC via USB-to-UART or USB-to-RS-232, launch AICS, and configure the following. If you have any problems with configuration or need custom settings please reach out to `Aeron Systems <support.ins@aeronsystems.com>`__.
...
* **Binary Strings (output enable)** - the following packets must all be enabled:
  ...
  * ``SENS_PARA`` - gyroscope, accelerometer, magnetometer and barometer.
...
* **Binary Data Rate** - all packets above must be configured as per specific vehicle chosen. The recommended setting depends on the vehicle type and on whether IMU data will be consumed by ArduPilot
  (See :ref:`use_as_external_sensor_set` below). Whatever the vehicle, these minimum rates must be met, or the autopilot will report the unit unhealthy and pre-arm will fail:

  * ``SENS_PARA`` - above 25 Hz (50 Hz recommended)
  * ``NAV_PARA1`` and ``NAV_PARA2`` - above 5 Hz (50 Hz recommended)
  * ``GPS_PARA`` and ``EXTD_GNSS`` - above 2 Hz (5 Hz recommended)
...
After setting these parameters, the configuration must be written to the device using **Save Configuration** in AICS so that it persists across power cycles.
...
* **External magnetometer aiding** - provides an absolute heading reference in GNSS-denied conditions. Must be enabled in AICS Sensor Settings.
* **External airspeed aiding** - improves dead-reckoning accuracy in GNSS-denied conditions for fixed-wing platforms. Must be enabled in AICS Sensor Settings.
...
There are two ways for ArduPilot to use the PLX3. You can choose between the two freely — the choice is a single parameter at runtime.
...
.. tip::
   ...
   sensor set (per the section above) so that you can switch between the two solutions by changing ``AHRS_EKF_TYPE``, without re-flashing.
...
The driver writes up to three streaming log messages. **AERN** and **AERS** are decimated to :ref:`EAHRS_LOG_RATE<EAHRS_LOG_RATE>` (default 10 Hz); **AERG** is written once per GNSS packet.
...
The **AERS** (Aeron Sensors) message contains sensor data from ``SENS_PARA``:
...
**The EAHRS_TYPE dropdown does not list "Aeron".**
Your firmware build does not include the Aeron driver. Either grab a firmware from the `Custom Build Server
<https://custom.ardupilot.org/add_build>`__ with External AHRS support enabled, or use an official build from 4.8 onwards.

**Pre-arm fails with "Aeron Unhealthy".**
The driver has not received a complete packet set within the staleness deadlines, or the PLX3 is reporting a hardware fault. Check that all five binary packets are enabled in AICS at the required rates and that the autopilot baud rate matches the PLX3 baud rate. If the GCS is also showing an ``Aeron:`` hardware fault message, the unit itself needs attention.

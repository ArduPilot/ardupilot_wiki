...
I'm facing an issue with the IMU SPI initialisation. To fix, you need to download and compile the spidev_test program and run prior ardupilot.
...
Now, let's create an auxiliar script to setting some linux stuffs ::
    ...
    #!/usr/bin/bash
    #This is needed to initalisation the SPI Bus for the IMU
...
With the `pull request <https://github.com/ArduPilot/ardupilot/pull/32012>`__
...
You need to enable and up the can network interface, in my case I added the following lines at the prep_ardu.sh script.
...
Something I point is the linux kernel. Beside all working and support from Mr, RobertCNelson, I noted sometimes a new kernel can add problems with the SPI bus. This is noted when you run ardupilot and receive messages of the IMU RESET in the GCS messages panel.

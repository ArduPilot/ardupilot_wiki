...
def generate_rst_files(commits_to_checkout_and_parse):
    ...
    def replace_anchors(source_file, dest_file, version_tag):
        ...
        for line in file_in:
            if (re.match("(^.. _)(.*):$", line)) and ("latest" not in version_tag):
                ...
                out_line += "\n.. raw:: html\n\n"
                out_line += (
                    "   <h2>Full Parameter List of " + version_tag[1:].replace("-", " ") + "</h2>\n\n"
    ...
    for i in commits_to_checkout_and_parse:
        ...
        try:
            ...
            if (
                ('rover' in vehicle.lower()) and ('v3.' not in version.lower()) and ('v4.0' not in version.lower())
            ):  # Workaround the vehicle renaming (Rover, APMRover2 ArduRover...)  # noqa: E501
                os.system("python3 ./param_parse.py --vehicle " + 'Rover')
            else:  # regular case
                os.system(
                    "python3 ./param_parse.py --vehicle " + vehicle_new_to_old_name[vehicle]
                )  # option "param_parse.py --format rst" is not available in all commits where param_parse.py is found  # noqa: E501
        ...
        for filename in parameters_files:
            if "beta" in filename or "rc" in filename:  # Plane uses BETA, Copter and Rover uses RCn
                ...
                json_lines.append(
                    ...
                    + "\" : \""
                    + ("parameters.html\"")
                )  # Trying to re-enable toc list on the left bar on the wiki by forcing latest file name.  # noqa: E501

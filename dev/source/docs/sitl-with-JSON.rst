...
with easily configurable fields. The interface uses UDP communication and is launched using ``sim_vehichle.py -f JSON:127.0.0.1`` or with the vehicle executable ``--model json:127.0.0.1`` where, if necessary, ``127.0.0.1`` is replaced 

...
- defaults.parm if board specific defaults are needed. Note do not define things already defaulted. Put Serial port protocol default changes, Battery monitor params, and Relay default boot states (``RELAY1_DEFAULT`` through ``RELAY6_DEFAULT``) in the hwdef file, not in the defaults.param file.

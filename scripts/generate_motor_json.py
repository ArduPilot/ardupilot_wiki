...
def fetch_branches(owner, repo, per_page=100):
    ...
    while url:
        ...
        try:
            ...
            data = response.json()
            branches.extend([{"name": branch["name"], "sha": branch["commit"]["sha"]} for branch in data])
...
def fetch_required_files():
    ...
    """
    latest_copter = get_latest_branch(AP_GITHUB_OWNER, AP_GITHUB_REPO, prefix=AP_COPTER_BRANCH)
    ...
    if latest_copter:
        ...
        motors_class_h = fetch_github_raw_file(AP_GITHUB_OWNER, AP_GITHUB_REPO, latest_copter["sha"], AP_MOTORS_CLASS_H)
        motors_matrix_cpp = fetch_github_raw_file(AP_GITHUB_OWNER, AP_GITHUB_REPO, latest_copter["sha"], AP_MOTORS_MATRIX_CPP)
...
def parse_header_matches(cpp_header, pattern, delimiter):
    ...
    return {match.split(delimiter)[0].strip(): int(match.split(delimiter)[1]) for match in matches}
...
def normalize_rpy_factors(frame_type_detail):
    ...
    for motor in frame_type_detail["motors"]:
        motor["Roll"] = motor["Roll"] if roll_factor == 0 else truncate(0.5 * motor["Roll"] / roll_factor, 4)
        motor["Pitch"] = motor["Pitch"] if pitch_factor == 0 else truncate(0.5 * motor["Pitch"] / pitch_factor, 4)
    ...
    for (
        ...
        for (
            ...
            if len(layout["motors"]) == 0:
                layout["motors"] += parse_motor_block(frame_type_block, add_single_motor_pattern)
            if len(layout["motors"]) == 0:
                layout["motors"] += parse_motor_block(frame_type_block, add_motors_raw_pattern, is_raw=True)
...
def generate_json():
    ...
    motor_matrices = {
        ...
        "layouts": generate_motor_matrices(ap_motors_matrix_cpp, frame_class_defs, frame_type_defs),
    }
...
if __name__ == "__main__":
    ...
    """
    parser = argparse.ArgumentParser(description="Generate motor matrices (JSON) from 'AP_Motors/AP_MotorsMatrix.cpp'.")
    parser.add_argument(
        "-o",
        "--output",
        nargs="?",

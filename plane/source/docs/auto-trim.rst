...
The amount that can be trimmed is limited to about 20% of the throw around the current trim setting depending on total ``SEROVX_MAX/MIN`` range, so very radical out-of-trim conditions are not correctable. Also, it will not be updated unless there is no pilot input, the plane is being controlled by the autopilot to be in generally level flight, and the plane is flying above 8m/s ground speed.

For example if the elevator servo has the following values for min/max/trim, 1100/1900/1500, then the auto trim can change the trim value to 1420 to 1580us.

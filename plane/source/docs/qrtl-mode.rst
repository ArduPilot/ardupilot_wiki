...
The QuadPlane will immediately navigate towards the home location at ``Q_WP_SPEED``, climbing or descending towards the :ref:`Q_RTL_ALT<Q_RTL_ALT>` altitude. Once arriving within ``Q_WP_RADIUS`` distance of home, it will begin descending at ``Q_WP_SPEED_DN`` rate, until it reaches :ref:`Q_LAND_FINAL_ALT<Q_LAND_FINAL_ALT>` at which point it will descend at :ref:`Q_LAND_FINAL_SPD<Q_LAND_FINAL_SPD>` until landing.

...
.. note:: The airspeed used for transition can be found in dataflash logs as CTUN.As. This is a canonical value which will incude the airspeed sensor if enabled, or use the synthetic airspeed if not, so if you are having trouble acheiving the transition speed, this is the value you should refer to.

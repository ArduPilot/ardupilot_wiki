...
- in a fixed wing mode when further than 2 times the greater of either :ref:`RTL_RADIUS<RTL_RADIUS>` or :ref:`WP_LOITER_RADS<WP_LOITER_RAD>` (MAXRAD) **plus** a calculated distance needed to descend from :ref:`RTL_ALTITUDE<RTL_ALTITUDE>` to :ref:`Q_RTL_ALT<Q_RTL_ALT>`, the plane will head for :ref:`RTL_ALTITUDE<RTL_ALTITUDE>`. If it is above that altitude when the approach begins, it does not dive for it: instead the target altitude ramps down linearly with distance from wherever the vehicle was when the approach started, reaching :ref:`RTL_ALTITUDE<RTL_ALTITUDE>` at exactly the distance where the descent to :ref:`Q_RTL_ALT<Q_RTL_ALT>` begins, so the two slopes join into a single continuous descent. If it is at or below :ref:`RTL_ALTITUDE<RTL_ALTITUDE>` it will climb towards it as before. If within that range, it will attempt to climb/descend to a linear descent slope, meet it, and continue to descend, as shown above.

  .. note:: The gradual descent to :ref:`RTL_ALTITUDE<RTL_ALTITUDE>` is gated by :ref:`ALT_SLOPE_MIN<ALT_SLOPE_MIN>`, just as fixed wing waypoint altitude changes are: if :ref:`ALT_SLOPE_MIN<ALT_SLOPE_MIN>` is zero, or the vehicle is less than that many meters above :ref:`RTL_ALTITUDE<RTL_ALTITUDE>`, the target moves to :ref:`RTL_ALTITUDE<RTL_ALTITUDE>` immediately instead. If the vehicle is already sinking faster than the ramp, for example if QRTL was entered while descending steeply, it is not commanded to climb back up to the ramp: the target follows the vehicle down, but never below :ref:`RTL_ALTITUDE<RTL_ALTITUDE>`.
...
:ref:`Q_WP_SPD<Q_WP_SPD>` speed towards the return point, at an altitude of
